...
public class DynamoDBMapperQueryScanExample {
    ...
    static AWSCredentials awsCredentials = new BasicAWSCredentials(accessId, secretKey);
    ...
    private static void queryFilter(DynamoDBMapper mapper, String forumName, String threadSubject) {
        String partitionKey = forumName + "#" + threadSubject;

        final Map<String, Condition> queryFilterMap = new HashMap<>();
        final Condition queryFilterCondition = new Condition().withComparisonOperator(ComparisonOperator.EQ.toString())
            .withAttributeValueList(new AttributeValue().withS("User A"));
        queryFilterMap.put("PostedBy", queryFilterCondition);

        final Reply replyForQuery = new Reply();
        replyForQuery.setId(partitionKey);
        DynamoDBQueryExpression<Reply> queryExpression = new DynamoDBQueryExpression<Reply>()
            .withHashKeyValues(replyForQuery)
            .withQueryFilter(queryFilterMap);

        List<Reply> replies = mapper.query(Reply.class, queryExpression);
